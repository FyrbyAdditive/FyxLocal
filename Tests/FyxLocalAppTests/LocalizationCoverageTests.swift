...
struct LocalizationCoverageTests {
    ...
    private let requiredLanguages = ["en", "en-GB", "sv", "da", "nb", "es"]
    ...
    private func catalogURL() -> URL {
        // Climb from this test file to the repo root, then into Sources. We
        // locate the root by walking up until we find the `Sources` directory,
        // which is robust to the on-disk repo folder name (it is not the app
        // name — the working copy is checked out as "F-Chat").
        var url = URL(fileURLWithPath: #file)
        while url.path != "/"
            && !FileManager.default.fileExists(
                atPath: url.appendingPathComponent("Sources").path) {
            url.deleteLastPathComponent()
        }
        return url
            .appendingPathComponent("Sources")
            .appendingPathComponent("FyxLocalApp")
