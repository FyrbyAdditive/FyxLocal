...
public struct RequestPayloadBuilder: Sendable {
    ...
    private func applyClearing(to items: [InputItem], options: ClearOptions) -> [InputItem] {
        ...
        if cleared > 0 {
            FileHandle.standardError.write(Data(
                "[FyxLocal] cleared \(cleared) older tool result(s); projected tokens now \(total) (trigger \(options.triggerTokens))\n".utf8
            ))
        }
        return mutated
    }
    ...
    private func clearedPlaceholderJSON(callID: String, originalTokens: Int) -> String {
        ...
        return "{\"_fchat_cleared\":true,\"call_id\":\"\(safeCallID)\",\"original_tokens\":\(originalTokens),\"hint\":\"This tool result was cleared by FyxLocal to save context. Re-call the tool with the same arguments to refetch.\"}"
    }
}
