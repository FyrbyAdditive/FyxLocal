...
final class AppEnvironment {
    ...
    init() {
        // First-launch rebrand migration (FyxLocal → FyxLocal): move the data
        // directory and copy Keychain secrets across the bundle-id change, so
        // upgrading users keep their conversations, settings, API keys and
        // OAuth tokens. Must run before the first data/secret access below.
        ...
        do {
            ...
            FileHandle.standardError.write(Data("[FyxLocal] rag database open failed (\(error)); falling back to in-memory store\n".utf8))
    ...
    func persistNow() {
        ...
        Task.detached(priority: .utility) {
            do {
                ...
                FileHandle.standardError.write(Data("[FyxLocal] persist failed: \(error)\n".utf8))
            }
        }
    }
...
/// "About FyxLocal" menu item opens Settings on the `.about` tab instead of the
