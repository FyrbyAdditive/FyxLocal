...
private struct AboutTab: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            // FyxLocal logo + name/version/description, vertically centered in
            ...
            VStack(spacing: 12) {
                ...
                Text("FyxLocal").font(.title.bold())
                Text("Version \(FyxLocal.version)")
                    .foregroundStyle(.secondary)
                Text("Native macOS LLM chat client.")
                    .foregroundStyle(.secondary)

                // Licensing: FyxLocal is GPLv3; third-party components are credited
                ...
                VStack(spacing: 4) {
                    Text("Free software under the GNU GPL v3.0.")
                        .foregroundStyle(.secondary)
                    HStack(spacing: 10) {
                        if let source = URL(string: "https://github.com/FyrbyAdditive/FyxLocal") {
    ...
    /// Load a PNG from the FyxLocalApp SPM resource bundle as an NSImage.
