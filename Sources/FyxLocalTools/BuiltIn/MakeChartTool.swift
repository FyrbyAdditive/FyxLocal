...
/// (`ToolChartView` in FyxLocalApp) then renders it as a Swift Charts bar /
...
/// validation) and the UI (for rendering). Lives in FyxLocalTools so both
/// FyxLocalTools tests and the FyxLocalApp renderer can import it; the renderer
/// itself is in FyxLocalApp because Swift Charts is a UI dependency.
